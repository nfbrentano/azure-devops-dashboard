/**
 * UI Translations for Azure DevOps Dashboard
 */
import type { Translations } from './types.ts';

export const translations: Translations = {
    en: {
        'tab-dashboard': 'Dashboard',
        'tab-items': 'Item Details',
        'tab-timeline': 'Timeline',
        'tab-setup': 'Setup',
        'btn-logout': 'Logout',
        'btn-refresh': 'Refresh Data',
        'btn-unlock': 'Unlock',
        'setup-title': 'Azure DevOps Dashboard',
        'setup-subtitle': 'Connect your account to view metrics',
        'label-org': 'Organization',
        'label-project': 'Project',
        'label-pat': 'Personal Access Token',
        'label-security-password': 'Security Password',
        'label-company-name': 'Company Name (Watermark)',
        'label-save-credentials': 'Save credentials in this browser',
        'unlock-title': 'Dashboard Locked',
        'unlock-subtitle': 'Enter your security password to unlock',
        'dashboard-title': 'Development Analytics',
        'items-view-title': 'Item Details & Aging',
        'active-query-placeholder': 'Select a query to start',
        'chart-comparison-title': 'Lead vs Cycle Time Comparison',
        'chart-throughput-title': 'Throughput (Items per Week) — Requirement Backlog',
        'portfolio-section-title': 'Features & Portfolio',
        'gantt-section-title': 'Hierarchical Gantt',
        'timeline-gantt-title': 'Hierarchical Gantt',
        'timeline-section-title': 'Project Timeline (Portfolio)',
        'timeline-section-subtitle': 'Roadmap view of all Portfolio Work Items (Epics, Features) in the configured project',
        'timeline-filter-types': 'Work Item Types',
        'timeline-filter-states': 'Work Item States',
        'gantt-period-total': 'General View (Auto)',
        'gantt-period-year': 'Yearly',
        'gantt-period-trimester': 'Trimester',
        'gantt-period-bimester': 'Bimester',
        'gantt-period-month': 'Monthly',
        'gantt-period-week': 'Weekly',
        'legend-type-title': 'Item Classification',
        'legend-status-title': 'Delivery Status (Bar Color)',
        'label-query': 'Query',
        'label-delivered': 'Delivered',
        'label-items': 'items',
        'status-backlog': 'Backlog',
        'status-inprogress': 'In Progress',
        'status-done': 'Done',
        'status-removed': 'Removed',
        'status-total': 'Total Items',
        'status-done-removed': 'Done / Removed',
        'btn-export-chart': 'Export Chart (PNG)',
        'msg-auth-failed': 'Failed to connect. Please check your credentials.',
        'msg-no-items': 'No items found for the selected query.',
        'theme-toggle-title': 'Toggle Dark/Light Mode',
        'refresh-btn-title': 'Refresh Data',
        'query-selector-placeholder': 'Select Analytics Query',
        'lang-toggle-title': 'Switch to Portuguese',
        // Metrics Guide
        'metrics-guide-title': 'Metrics Guide',
        'metric-lead-time-title': 'Lead Time',
        'metric-lead-time-desc':
            'Measures the total time from when an item is <strong>created</strong> until it is <strong>completed</strong>. Reflects the full customer experience.',
        'metric-cycle-time-title': 'Cycle Time',
        'metric-cycle-time-desc':
            "Measures the time an item was actually <strong>in development</strong> (from 'In Progress' until completion). Focuses on execution efficiency.",
        'metric-comparison-title': 'Lead vs Cycle Time',
        'metric-comparison-desc':
            'Compares the average Lead Time and Cycle Time of the current period with the previous, allowing to identify trends or bottlenecks.',
        'metric-portfolio-title': 'Features & Portfolio',
        'metric-portfolio-desc':
            'Macro view of Epic and Feature progress, showing how items are distributed across the workflow stages.',
        'global-filter-title': 'Item Types Displayed',
        'metric-aging-title': 'Work Item Aging',
        'metric-aging-desc':
            'Measures how many days an <strong>In Progress</strong> item has been without updates. Helps identify stuck items or bottlenecks.',
        'metric-gantt-title': 'Hierarchical Gantt',
        'metric-gantt-desc':
            'Hierarchical timeline showing item duration and nesting, allowing to visualize project sequencing and dependencies.',
        'chart-aging-title': 'Work Item Aging (In Progress)',
        'chart-assignee-title': 'Workload by Assignee',
        'chart-cfd-title': 'Cumulative Flow Diagram (CFD)',
        'chart-activity-title': 'Delivery Activity (All Time)',
        'chart-wip-title': 'WIP by Kanban Column',
        'msg-wip-empty': 'No items found in Kanban columns.',
        'gantt-missing-dates-tooltip': 'Missing Start or Target date',
        'aria-theme-toggle': 'Toggle dark and light mode',
        'aria-lang-toggle': 'Switch language to Portuguese',
        'aria-refresh': 'Refresh data from Azure DevOps',
        'aria-gantt-prev': 'Previous period',
        'aria-gantt-next': 'Next period',
        'items-view-subtitle': 'Detailed analysis of item inactivity and health',
        'empty-state-title': 'No items found',
        'empty-state-desc':
            'The selected query returned no work items. Please check your query or try a different one.',
        'setup-security-warning-title': 'GitHub Pages Security',
        'setup-security-warning-text':
            'As this is a static application, your PAT is sent directly from the browser to Azure. Use a short-lived PAT for security.',
        'placeholder-org': 'ex: my-org',
        'placeholder-project': 'ex: my-project',
        'placeholder-pat': 'Your Azure DevOps Token',
        'placeholder-security-password': 'Used to encrypt your data',
        'placeholder-company-name': 'Ex: My Company S.A.',
        'placeholder-unlock-password': 'Your security password',
        'loading-label': 'Loading...',
        'msg-error-loading': 'Error loading data',
        'msg-metadata-failed': 'Failed to fetch Azure DevOps metadata',
        'msg-invalid-password': 'Incorrect password',
        'msg-partial-data-single': 'A portion of the data could not be loaded. Partial data displayed.',
        'msg-partial-data-multiple': '{count} portions of data failed to load. Partial data displayed.',
        'btn-connect-load': 'Connect & Load',
        'link-setup-guide': 'Need help finding these?',
        'setup-guide-explanation': 'Step-by-step guide to get your Token, Organization and Project.',
        'link-forgot-password': 'Clear data and logout',
        'label-days': 'Days',
        'label-days-inactive': 'Work Item Aging (Days)',
        'label-days-since-update': 'Days since last update',
        'label-number-of-items': 'Number of Items',
        'label-items-count': 'Items Count',
        'label-quantity': 'Quantity',
        'label-delivered-items': 'Delivered Items',
        'msg-aging-empty': 'No in-progress items found for aging analysis.',
        'msg-assignee-empty': 'No assignee data available.',
        'msg-cfd-empty': 'No data available for Cumulative Flow Diagram.',
        'msg-portfolio-empty': 'No portfolio items found for the selected status.',
        'msg-gantt-empty': 'No items available to display on the Gantt chart for this period.',
        'msg-missing-config': 'Azure DevOps configuration is missing. Please complete the setup.',
        'label-no-title': 'No Title',
        'label-unassigned': 'Unassigned',
        'label-week-short': 'W',
        'heatmap-days': 'Sun,Mon,Tue,Wed,Thu,Fri,Sat',
        'chart-bottlenecks-title': 'Work Item Bottlenecks (Avg Time per Column)',
        'label-avg-days': 'Avg Days',
        'label-no-planned-dates': 'No dates planned',
        'msg-bottlenecks-empty': 'No data available for bottleneck analysis.',
        'loading-fetching-query': 'Fetching query...',
        'loading-items': 'Loading {count} items...',
        'loading-history': 'Loading history for {count} items...',
        'loading-portfolio': 'Fetching project Portfolio (Initiatives/Epics/Features)...',
        'msg-portfolio-empty-toast': 'No items found in the portfolio.',
        'loading-processing-items': 'Processing {count} items...',
        'msg-timeline-error': 'Timeline error: {message}',
        'label-load-more': 'Load More',
        'cfd-period-30': 'Last 30 Days',
        'cfd-period-60': 'Last 60 Days',
        'cfd-period-90': 'Last 90 Days',
        'cfd-period-180': 'Last 180 Days',
        'alert-stale-title': 'Stuck In-Progress Items',
        'alert-stale-msg': 'items in In Progress have had no updates for more than 14 days.',
        'alert-wip-title': 'WIP Limit Warning',
        'alert-wip-msg': 'Columns exceeding recommended WIP limit:',
        'alert-bottleneck-title': 'Main Bottleneck Detected',
        'alert-bottleneck-msg': 'The column',
        'placeholder-search-items': 'Search by title, ID or assignee...',
        'btn-export-pdf': 'Export PDF Report',
        'btn-export-csv': 'Export CSV',
        'chart-scatter-title': 'Cycle Time Scatter Plot & Percentiles',
        'scatter-item-label': 'Work Items (Cycle Time)',
        'label-completion-date': 'Completion Date',
        'label-cycle-time-days': 'Cycle Time (Days)',
        'shortcuts-title': 'Keyboard Shortcuts',
        'shortcut-tabs': 'Switch tabs (1: Dashboard, 2: Items, 3: Timeline, 4: Setup)',
        'shortcut-refresh': 'Refresh current query data',
        'shortcut-theme': 'Toggle Dark / Light theme',
        'shortcut-lang': 'Toggle Language (PT-BR / EN)',
        'shortcut-tv': 'Toggle Presentation / TV Mode',
        'shortcut-help': 'Show keyboard shortcuts help',
        'shortcut-esc': 'Close modals / Exit TV Mode',
        'btn-tv-mode': 'Presentation Mode (TV)',
        'tv-mode-active': 'TV Mode Active. Press ESC or F to exit.',
        'btn-share-url': 'Share View (Copy Link)',
        'msg-share-copied': 'Dashboard link copied to clipboard!',
        'sla-title': 'Service Level Agreements (SLA)',
        'sla-target': 'Target',
        'sla-within-target': 'within SLA target',
        'label-avg': 'Average',
        'alerts-btn-title': 'Configure Alerts',
        'alerts-modal-title': 'Alert Rules & Thresholds',
        'alerts-save': 'Save Rules',
        'alerts-reset': 'Reset Defaults'
    },
    'pt-br': {
        'tab-dashboard': 'Painel',
        'tab-items': 'Detalhes de Itens',
        'tab-timeline': 'Linha do Tempo',
        'tab-setup': 'Configuração',
        'setup-title': 'Dashboard Azure DevOps',
        'setup-subtitle': 'Conecte sua conta para visualizar métricas',
        'label-org': 'Organização',
        'label-project': 'Projeto',
        'label-pat': 'Personal Access Token (PAT)',
        'label-security-password': 'Senha de Segurança',
        'label-company-name': "Nome da Empresa (Marca d'água)",
        'label-save-credentials': 'Salvar credenciais neste navegador',
        'btn-logout': 'Sair da Conta',
        'dashboard-title': 'Análise de Desenvolvimento',
        'active-query-placeholder': 'Selecione uma consulta para começar',
        'chart-comparison-title': 'Comparação Lead vs Cycle Time',
        'chart-throughput-title': 'Throughput (Itens por Semana) — Backlog de Requisitos',
        'portfolio-section-title': 'Features & Portfólio',
        'gantt-section-title': 'Gantt Hierárquico',
        'timeline-gantt-title': 'Gantt Hierárquico',
        'timeline-section-title': 'Linha do Tempo do Projeto (Portfólio)',
        'timeline-section-subtitle': 'Visão consolidada de Épicos e Features do projeto configurado',
        'timeline-filter-types': 'Tipos de Itens',
        'timeline-filter-states': 'Status dos Itens',
        'gantt-period-total': 'Visão Geral (Auto)',
        'gantt-period-year': 'Anual',
        'gantt-period-trimester': 'Trimestral',
        'gantt-period-bimester': 'Bimestral',
        'gantt-period-month': 'Mensal',
        'gantt-period-week': 'Semanal',
        'legend-type-title': 'Classificação de Itens',
        'legend-status-title': 'Status de Entrega (Cor da Barra)',
        'label-query': 'Consulta',
        'label-delivered': 'Entregue',
        'label-items': 'itens',
        'status-backlog': 'Backlog',
        'status-inprogress': 'Em Progresso',
        'status-done': 'Concluído',
        'status-removed': 'Removido',
        'status-total': 'Total de Itens',
        'status-done-removed': 'Concluído / Removido',
        'btn-export-chart': 'Exportar Gráfico (PNG)',
        'msg-auth-failed': 'Erro na conexão. Verifique suas credenciais.',
        'msg-no-items': 'Nenhum item encontrado para esta consulta.',
        'theme-toggle-title': 'Alternar Tema Claro/Escuro',
        'refresh-btn-title': 'Atualizar Dados',
        'query-selector-placeholder': 'Selecione a Consulta de Analytics',
        'lang-toggle-title': 'Mudar para Inglês',
        // Metrics Guide
        'metrics-guide-title': 'Guia de Métricas',
        'metric-lead-time-title': 'Lead Time',
        'metric-lead-time-desc':
            'Mede o tempo total desde que um item é <strong>criado</strong> até ser <strong>concluído</strong>. Reflete a experiência completa do cliente.',
        'metric-cycle-time-title': 'Cycle Time',
        'metric-cycle-time-desc':
            "Mede o tempo em que o item esteve efetivamente <strong>em desenvolvimento</strong> (de 'Em Progresso' até a conclusão). Foca na eficiência da execução.",
        'metric-comparison-title': 'Lead vs Cycle Time',
        'metric-comparison-desc':
            'Compara o Lead e Cycle Time médio do período atual com o anterior, permitindo identificar tendências ou gargalos.',
        'metric-portfolio-title': 'Features & Portfolio',
        'metric-portfolio-desc':
            'Visualização macro do progresso de Epics e Features, mostrando a distribuição dos itens por etapa do fluxo.',
        'global-filter-title': 'Tipos de Itens Visíveis',
        'metric-aging-title': 'Envelhecimento do Item (Work Item Aging)',
        'metric-aging-desc':
            'Mede quantos dias um item em <strong>Em Progresso</strong> está sem atualizações. Ajuda a identificar itens travados ou gargalos.',
        'metric-gantt-title': 'Gantt Hierárquico',
        'metric-gantt-desc':
            'Linha do tempo hierárquica que mostra a duração e o aninhamento dos itens, permitindo visualizar o sequenciamento e dependências.',
        'chart-aging-title': 'Envelhecimento dos Itens (Em Progresso)',
        'chart-assignee-title': 'Carga de Trabalho por Responsável',
        'chart-cfd-title': 'Diagrama de Fluxo Cumulativo (CFD)',
        'chart-activity-title': 'Atividade de Entrega (Todo o Período)',
        'chart-wip-title': 'WIP por Coluna Kanban',
        'msg-wip-empty': 'Nenhum item encontrado nas colunas do Kanban.',
        'gantt-missing-dates-tooltip': 'Data de Início ou Término ausente',
        'aria-theme-toggle': 'Alternar entre modo claro e escuro',
        'aria-lang-toggle': 'Alterar idioma para Inglês',
        'aria-refresh': 'Atualizar dados do Azure DevOps',
        'aria-gantt-prev': 'Período anterior',
        'aria-gantt-next': 'Próximo período',
        'items-view-title': 'Detalhes do Item e Envelhecimento',
        'items-view-subtitle': 'Análise detalhada de inatividade e saúde dos itens',
        'empty-state-title': 'Nenhum item encontrado',
        'empty-state-desc':
            'A consulta selecionada não retornou nenhum item de trabalho. Por favor, verifique sua consulta ou tente outra.',
        'setup-security-warning-title': 'Segurança para GitHub Pages',
        'setup-security-warning-text':
            'Como esta aplicação é estática, o seu PAT será enviado diretamente do navegador para o Azure. Use um PAT de curta duração por segurança.',
        'placeholder-org': 'ex: minha-org',
        'placeholder-project': 'ex: meu-projeto',
        'placeholder-pat': 'Seu Token do Azure DevOps',
        'placeholder-security-password': 'Usada para criptografar seus dados',
        'placeholder-company-name': 'Ex: Minha Empresa S.A.',
        'placeholder-unlock-password': 'Sua senha de segurança',
        'loading-label': 'Carregando...',
        'msg-error-loading': 'Erro ao carregar dados',
        'msg-metadata-failed': 'Falha ao buscar metadados do Azure DevOps',
        'msg-invalid-password': 'Senha incorreta',
        'msg-partial-data-single': 'Uma parte dos dados não pôde ser carregada. Dados parciais exibidos.',
        'msg-partial-data-multiple': '{count} partes dos dados falharam ao carregar. Dados parciais exibidos.',
        'btn-connect-load': 'Conectar e Carregar',
        'link-setup-guide': 'Precisa de ajuda para encontrar estes dados?',
        'setup-guide-explanation': 'Guia passo a passo para obter seu Token, Organização e Projeto.',
        'link-forgot-password': 'Limpar dados e sair',
        'label-days': 'Dias',
        'label-days-inactive': 'Envelhecimento do Item (Dias)',
        'label-days-since-update': 'Dias desde a última atualização',
        'label-number-of-items': 'Número de Itens',
        'label-items-count': 'Contagem de Itens',
        'label-quantity': 'Quantidade',
        'label-delivered-items': 'Itens Entregues',
        'msg-aging-empty': 'Nenhum item em progresso encontrado para análise de envelhecimento.',
        'msg-assignee-empty': 'Nenhum dado de responsável disponível.',
        'msg-cfd-empty': 'Nenhum dado disponível para o Diagrama de Fluxo Cumulativo.',
        'msg-portfolio-empty': 'Nenhum item de portfólio encontrado para os status selecionados.',
        'msg-gantt-empty': 'Nenhum item disponível para exibir no Gantt para este período.',
        'msg-missing-config': 'As configurações do Azure DevOps estão ausentes. Por favor, complete a configuração.',
        'label-no-title': 'Sem Título',
        'label-unassigned': 'Não Atribuído',
        'label-week-short': 'S',
        'heatmap-days': 'Dom,Seg,Ter,Qua,Qui,Sex,Sáb',
        'chart-bottlenecks-title': 'Gargalos de Work Items (Tempo Médio por Coluna)',
        'label-avg-days': 'Média de Dias',
        'label-no-planned-dates': 'Sem datas planejadas',
        'msg-bottlenecks-empty': 'Nenhum dado disponível para análise de gargalos.',
        'loading-fetching-query': 'Buscando consulta...',
        'loading-items': 'Carregando {count} itens...',
        'loading-history': 'Carregando histórico de {count} itens...',
        'loading-portfolio': 'Buscando Portfólio (Iniciativas/Épicos/Features) do projeto...',
        'msg-portfolio-empty-toast': 'Nenhum item encontrado no portfólio.',
        'loading-processing-items': 'Processando {count} itens...',
        'msg-timeline-error': 'Erro na Timeline: {message}',
        'label-load-more': 'Carregar Mais',
        'cfd-period-30': 'Últimos 30 Dias',
        'cfd-period-60': 'Últimos 60 Dias',
        'cfd-period-90': 'Últimos 90 Dias',
        'cfd-period-180': 'Últimos 180 Dias',
        'alert-stale-title': 'Itens Estagnados em Progresso',
        'alert-stale-msg': 'itens em Progresso estão há mais de 14 dias sem atualização.',
        'alert-wip-title': 'Alerta de Limite de WIP',
        'alert-wip-msg': 'Colunas com sobrecarga de itens:',
        'alert-bottleneck-title': 'Principal Gargalo Detectado',
        'alert-bottleneck-msg': 'A coluna',
        'placeholder-search-items': 'Buscar por título, ID ou responsável...',
        'btn-export-pdf': 'Exportar Relatório PDF',
        'btn-export-csv': 'Exportar CSV',
        'chart-scatter-title': 'Dispersão de Cycle Time & Percentis',
        'scatter-item-label': 'Work Items (Cycle Time)',
        'label-completion-date': 'Data de Conclusão',
        'label-cycle-time-days': 'Cycle Time (Dias)',
        'shortcuts-title': 'Atalhos de Teclado',
        'shortcut-tabs': 'Alternar abas (1: Painel, 2: Itens, 3: Timeline, 4: Setup)',
        'shortcut-refresh': 'Atualizar dados da consulta',
        'shortcut-theme': 'Alternar tema Claro / Escuro',
        'shortcut-lang': 'Alternar idioma (PT-BR / EN)',
        'shortcut-tv': 'Alternar Modo Apresentação / TV',
        'shortcut-help': 'Exibir atalhos de teclado',
        'shortcut-esc': 'Fechar modais / Sair do modo TV',
        'btn-tv-mode': 'Modo Apresentação (TV)',
        'tv-mode-active': 'Modo TV Ativo. Pressione ESC ou F para sair.',
        'btn-share-url': 'Compartilhar Visualização (Copiar Link)',
        'msg-share-copied': 'Link do dashboard copiado com filtros atuais!',
        'sla-title': 'Acordos de Nível de Serviço (SLA)',
        'sla-target': 'Meta',
        'sla-within-target': 'dentro da meta',
        'label-avg': 'Média',
        'alerts-btn-title': 'Configurar Alertas',
        'alerts-modal-title': 'Regras e Limites de Alertas',
        'alerts-save': 'Salvar Regras',
        'alerts-reset': 'Restaurar Padrões'
    }
};
